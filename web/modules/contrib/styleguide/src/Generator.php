...
/**
 * Generates output for rendered elements in the styleguide.
 *
...
class Generator implements GeneratorInterface {
  ...
  public function paragraphs($size = 5, $render = FALSE) {
    ...
    return $render ? $this->renderer->render($output) : $output;
  }
  ...
  public function image($image = 'vertical', $type = 'jpg', $min_resolution = '240x320', $max_resolution = '240x320') {
    $file_path = $this->configFactory->get('system.file')
  ...
  public function sentence($link = FALSE) {
    ...
    if ($link) {
      ...
      $rendered_link = $this->renderer->renderInIsolation($link);
  ...
  public function links($url, $size = 4) {
    ...
    for ($i = 0; $i < 5; $i++) {
      $links[] = [
        ...
        'fragment' => Html::getUniqueId('links'),
      ];
    }
    return $links;
  }
  ...
  public function ulLinks() {
    ...
    for ($i = 0; $i <= 10; $i++) {
      ...
      $links[$word] = [
        'title' => $word,
        'url' => Url::fromUserInput($this->requestStack->getCurrentRequest()->getRequestUri()),
        'fragment' => Html::getUniqueId('ul_links'),
      ];
    }
